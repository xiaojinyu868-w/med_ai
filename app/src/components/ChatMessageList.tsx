...
export const ChatMessageList = ({ messages, onOptionSelect, sending = false }: Props) => {
  return (
    <Stack spacing={2}>
      {messages.map((msg) => {
        ...
        if (msg.type === 'options') {
          return (
            <Paper
              key={msg.id}
              sx={{ p: 2, bgcolor: color, alignSelf: align, color: textColor, borderRadius: 3, minWidth: 220 }}
            >
        ...
        return (
          <Paper
            key={msg.id}
            sx={{ p: 2, bgcolor: color, alignSelf: align, color: textColor, borderRadius: 3, maxWidth: '80%' }}
          >
            <Typography variant="body2">{msg.content}</Typography>
            {msg.role === 'system' && (
              <Chip size="small" label="系统" sx={{ mt: 1, bgcolor: '#fff', color: 'text.primary' }} />
            )}
          </Paper>
        );
      })}
