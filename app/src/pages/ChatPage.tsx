...
export const ChatPage = () => {
  ...
  const { messages, status, inputMetrics, error } = useAppSelector(
    (s) => s.chat,
  );
  ...
  const handleSend = () => {
    ...
    dispatch(
      sendChatMessage({
        content: input.trim(),
        editCount: inputMetrics.editCount,
      }),
    );
    setInput("");
  ...
  const handleOptionSelect = (id: string, option: string) => {
    ...
    dispatch(
      sendChatMessage({ content: option, editCount: inputMetrics.editCount }),
    );
    dispatch(resetEditCount());
  };
